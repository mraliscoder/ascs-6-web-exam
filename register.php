...
if (isset($_POST['username']) && isset($_POST['password'])) {
    ...
    $pass = hash('sha512', $password); // I want some salt... But I'm not in Saint Petersburg
    
    $checkIfUserExists = mysqli_query($link, "SELECT * FROM users WHERE username='$username'");
    if (mysqli_num_rows($checkIfUserExists) > 0) {
        die(json_encode([ "error" => "Пользователь уже существует" ]));
    }

    // НЕ СПРАШИВАЙТЕ ЗАЧЕМ, ЭТО ПРОСТО ОРНО
    $checkIfPasswordExists = mysqli_query($link, "SELECT username FROM users WHERE password='$pass'");
    if (mysqli_num_rows($checkIfPasswordExists) > 0) {
        die(json_encode([ "error" => "Этот пароль уже используется пользователем " . mysqli_fetch_array($checkIfPasswordExists)["username"] ]));
    }

    mysqli_query($link, "INSERT INTO users(username,password) VALUES('$username', '$pass')");
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        ...
        <p class="text-center text-gray-600 text-sm mt-6">
            ...
            <a href="/login.php" class="text-blue-600 hover:text-blue-700 font-medium">Войти</a>
        </p>
    </div>
